#!/usr/bin/env python3
"""
BearBox — CONNECTED Screen
Plays when internet is restored.
Red → Blue color transition with glitch effects.
"""
...
class _BgCol:
    ...
    def draw(self, d, fnt, progress):
        for c in self.chars:
            y = int(c["y"])
            if 0 <= y <= H:
                # transition from red chars to blue chars
                r = int(60 * (1.0 - progress))
                b = int(60 * progress)
...
def run():
    ...
    while True:
        ...
        # background color: dark red → dark blue
        bg = (_lerp(15, 0, t), 0, _lerp(0, 20, t))

        img, d = new_frame(bg=bg)

        # scanlines
        scan_col = (_lerp(18, 0, t), 0, _lerp(0, 18, t))
        ...
        # main text — resolves from scrambled to correct
        result = ""
        ...
        text_col = _lerp_col((255, 40, 40), (100, 200, 255), t)
        ...
        if random.random() < GLITCH_CHANCE * 0.5:
            ...
            ghost_col = _lerp_col((80, 0, 0), (0, 30, 80), t)
            d.text((lx+ox, ly+2), result, font=F, fill=ghost_col)

        # glow
        glow = _lerp_col((30, 0, 0), (0, 10, 40), t)
        ...
        # underline
        ul_col = _lerp_col((120, 0, 0), (0, 100, 200), t)
        ...
        cx  = (W - cw) // 2
        cy  = ly + lh + 18
        cc  = _lerp_col((100, 20, 20), (40, 120, 200), t)
        d.text((cx, cy), CREDIT, font=Fc, fill=cc)

        push(img)
        time.sleep(1/30)

    # hold on final blue frame briefly
    img, d = new_frame(bg=(0, 0, 20))
    for y in range(0, H, 4):
        d.line([(0, y), (W, y)], fill=(0, 0, 18))
    lw = F.getbbox(LINE1)[2]
    lh = F.getbbox(LINE1)[3]
    d.text(((W-lw)//2, H//2-lh//2-14), LINE1, font=F, fill=(100, 200, 255))
    push(img)
    time.sleep(0.8)
