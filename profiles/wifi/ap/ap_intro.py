#!/usr/bin/env python3
"""BearBox AP — SIPHON ACTIVATED Intro"""
...
def _draw_logo(d, F, reveal_pct, alpha=1.0):
    ...
    for line, y in [(LINE1, l1_y), (LINE2, l2_y)]:
        ...
        for i, ch in enumerate(line):
            lp = max(0.0, min(1.0, (reveal_pct - (i / len(line)) * 0.4) * 2.5))
            if lp >= 1.0: result += ch
            elif lp > 0:  result += ch if random.random() < lp else random.choice(_CHARS)
            else:          result += random.choice(_CHARS)
        a   = int(255 * alpha)
        ...
        if reveal_pct > 0.7:
            ...
            d.line([(W//2 - lw//2, y + th(line) + 4),
                    (W//2 + lw//2, y + th(line) + 4)],
...
def run():
    ...
    while True:
        ...
        for col in cols: col.update()
        alpha = max(0.0, 1.0-(progress-HOLD_END)/(1.0-HOLD_END)) if progress > HOLD_END else 1.0
        img, d = new_frame(bg=C["bg"])
        draw_scanlines_pink(d)
        for col in cols: col.draw(d, bg_fnt)
        if progress > RESOLVE_START * 0.3:
            rp = max(0.0, min(1.0, (progress-RESOLVE_START*0.3)/(HOLD_START-RESOLVE_START*0.3)))
